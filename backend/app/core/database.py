...
def get_connection():
    \"\"\"Retorna conexao SQLite ativa com suporte a Row Factory.\"\"\"
...
def init_db():
    \"\"\"Garante que todas as tabelas necessarias estejam criadas.\"\"\"
    conn = get_connection()
    cur = conn.cursor()
    
    # 1. Tabela Principal de Notas / Titulos
    cur.execute(\"\"\"
    ...
    # 4. Tabela de Comprovantes de Adiantamento
    cur.execute(\"\"\"
